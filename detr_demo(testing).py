...
def plot_results(image, logits, boxes, fontsize):
    ...
    for logit, box in zip(logits, boxes):
        ...
        count += 1
        label = CLASSES[cls]
        box = box * t.Tensor([image.width, image.height, image.width, image.height]).cuda()  # scale up the box to the original size
        x, y, w, h = box
        x0, x1 = x - w//2, x + w//2
        y0, y1 = y - h//2, y + h//2
...
if __name__ == '__main__':
    ...
    model.eval().cuda()
    model = model
    input_folder = "C:/Users/devan/Downloads/detr-demo-master/detr-demo-master/images"
    ...
    with open("results.txt", "w") as f:
        for filename in os.listdir(input_folder):
                  if filename.endswith('.jpg'):
                    ...
                    img, img_tensor = load_image_local(image_path)
                    ...
                    img_cp.save(output_path)
                    print(f"{filename} inference time: {inference_time:.2f}s")  # print image name and inference 
                    f.write(f"{filename},{inference_time:.2f}\n")
